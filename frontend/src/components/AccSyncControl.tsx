...
import type { AccSyncStatusKind } from '../types';
import { Tooltip } from './ui/Tooltip';

/**
 * ACC game-directory sync control (Phase 2).
 *
 * A status icon in the ACC Track Library header. It auto-detects the MoTeC
 * export folder, lets the user (re)point it, enable/disable sync, and run an
 * on-demand import ("Sync now"). Only rendered for the ACC library.
 */

const STATUS_META: Record<AccSyncStatusKind, { label: string; tone: string; ring: string }> = {
...
const StatusIcon = ({ status, scanning }: { status: AccSyncStatusKind; scanning: boolean }) => {
...
export const AccSyncControl = () => {
    const accSync = useTelemetryStore(s => s.accSync);
    const isScanning = useTelemetryStore(s => s.isAccScanning);
    const fetchStatus = useTelemetryStore(s => s.fetchAccSyncStatus);
    const detectFolder = useTelemetryStore(s => s.detectAccFolder);
    const setConfig = useTelemetryStore(s => s.setAccSyncConfig);
    const triggerScan = useTelemetryStore(s => s.triggerAccScan);
    ...
    useEffect(() => { if (!accSync) fetchStatus(); }, [accSync, fetchStatus]);
    useEffect(() => { setFolderInput(accSync?.folder || ''); }, [accSync?.folder]);
    ...
    if (!accSync) return null;

    const status = isScanning ? 'scanning' : accSync.status;
    const meta = STATUS_META[status] ?? STATUS_META.unconfigured;
    const configured = !!accSync.folder;
    ...
    const handleDetect = () => run(async () => {
        ...
        if (!path) setErrorMsg('Could not auto-detect the ACC MoTeC folder. Enter it manually below.');
    });
    ...
    const result = accSync.lastResult;
                ...
                {open && (
                    ...
                    >
                        ...
                        <p className="text-[11px] text-gray-500 mb-3 leading-snug">
                            Automatically imports ACC MoTeC <span className="font-mono text-gray-400">.ld</span> recordings from your game folder.
                        </p>

                        {/* Folder path */}
                        <label className="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">MoTeC Folder</label>
                        <div className="flex items-center gap-2 mb-1">
                            <input
                                value={folderInput}
                                onChange={(e) => setFolderInput(e.target.value)}
                                placeholder="/path/to/…/Assetto Corsa Competizione/MoTeC"
                                spellCheck={false}
                            ...
                            <button
                                ...
                                disabled={busy || folderInput.trim() === (accSync.folder || '')}
                        ...
                        <div className="flex items-center justify-between mb-4">
                            ...
                            {accSync.autoDetected && configured && (
                        ...
                        <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between text-[10px]">
                            ...
                            <span className="text-gray-600 font-mono">{timeAgo(accSync.lastScanAt)}</span>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
