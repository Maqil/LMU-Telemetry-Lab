...
const ComparisonRow = ({
  ...
  return (
    ...
    >
      <div
        className={`glass-content grid ${compact ? "grid-cols-[20px_1fr_45px_1fr] gap-1 py-1.5 px-2" : "grid-cols-[30px_1fr_50px_1fr] gap-2 py-2.5 px-4"} items-center whitespace-nowrap cursor-default`}
      >
        <span
          className={`text-gray-500 ${compact ? "text-[8px]" : "text-[9px]"} font-black uppercase flex justify-center group-hover/row:text-gray-300 transition-colors`}
        >
          {label}
        </span>
        ...
        <span className={`text-amber-500/80 font-mono font-black ${compact ? "text-[11px]" : "text-[13px]"} text-left drop-shadow-sm`}>
          {formatTime(refVal)}
        </span>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!compact && (
          <span className="text-[11px] font-bold text-gray-400 uppercase tracking-tight group-hover/teleRow:text-gray-200 transition-colors">{label}</span>
        )}
...
const TyreDashboard = React.memo(
  ({
    data,
    cursorIndex,
    carClass,
    ...
    const getCompoundInfo = (idx: number | null) => {
      ...
      if (isHypercar) {
        switch (roundIdx) {
          case 0:
            return { label: "S", color: "text-red-500" };
          case 1:
            return { label: "M", color: "text-yellow-500" };
          case 2:
            return { label: "H", color: "text-white" };
          case 3:
            return { label: "W", color: "text-blue-500" };
          default:
    ...
    const WheelInfo = ({ wheelIdx, label: wLabel, className: wClassName = "" }: { wheelIdx: number; label: string; className?: string }) => {
      ...
      return (
        <div
          className={`relative flex flex-col items-center ${compact ? "p-1.5" : "p-2.5"} rounded-md transition-all duration-300 group/wheel glass-container-flat hover:bg-white/10 hover:shadow-[0_0_20px_rgba(255,255,255,0.05)] ring-1 ring-inset ring-white/5 min-w-0 ${wClassName}`}
          onMouseMove={(e) => handleGlassMouseMove(e, 0.2)}
        >
          <div className="glass-content w-full flex flex-col items-center">
            {isDetached && (
              <div
                className={`absolute inset-0 z-20 bg-red-900/80 rounded-xl flex items-center justify-center border-2 border-red-500 animate-pulse box-content -m-0.5`}
              >
            ...
            <div className="flex flex-col items-center gap-0.5 w-full border-t border-white/10 pt-1.5">
              <div
                className={`${compact ? "text-[11px]" : "text-sm"} font-black text-white font-mono drop-shadow-[0_0_10px_rgba(255,255,255,0.3)] tracking-tight`}
              >
              ...
              {!compact && (
                <div className="flex items-center gap-1 opacity-80">
                  ...
                  <span className="text-[9px] font-bold text-gray-400 font-mono">
                    {brakeTemp ? (tempUnit === "f" ? brakeTemp * 1.8 + 32 : brakeTemp).toFixed(0) : "--"}°
                  </span>
                </div>
              )}
            </div>
            <div
              className={`absolute -top-1 -left-1 bg-gray-800/90 text-gray-400 ${compact ? "text-[6px] px-1" : "text-[8px] px-1.5"} font-black py-0.5 rounded border border-white/10 uppercase tracking-tighter backdrop-blur-md`}
            >
              {wLabel}
            </div>
          </div>
    ...
    return (
      ...
      >
        <div className={`absolute top-0 left-0 flex items-center gap-1.5 ${compact ? "p-2" : "p-3"}`}>
          ...
          <span
            className={`${compact ? "text-[8px]" : "text-[10px]"} font-black uppercase tracking-[0.2em] ${isRef ? "text-amber-500" : "text-gray-500"} group-hover/tyres:text-white transition-colors`}
          >
...
const FuelDashboard = React.memo(
  ({
    data,
    cursorIndex,
    fuelCapacity,
    ...
    for (let i = 0; i < allFuel.length; i++) {
      if (allFuel[i] > maxFuelInData) maxFuelInData = allFuel[i];
    }
    ...
    return (
      ...
      >
        <div className={`absolute top-0 left-0 flex items-center gap-1.5 ${compact ? "p-2" : "p-3"}`}>
          <div className={`w-1.5 h-1.5 rounded-full ${isRef ? "bg-amber-400 animate-pulse" : "bg-blue-400"}`} />
          <span
            className={`${compact ? "text-[8px]" : "text-[10px]"} font-black uppercase tracking-[0.2em] ${isRef ? "text-amber-500" : "text-gray-500"} group-hover/fuel:text-white transition-colors`}
          >
            {isRef ? "Ref Fuel" : "Fuel"}
          </span>
        </div>
        <div className={`glass-content w-full flex items-center ${compact ? "gap-2" : "gap-5"}`}>
          {!compact && (
            <div className="p-2 bg-white/5 rounded-xl border border-white/10 group-hover:border-blue-500/40 group-hover:bg-white/10 transition-all">
              <img
                ...
                width={22}
                height={22}
                className={`opacity-90 object-contain w-4 h-4 shrink-0 pointer-events-none ${isRef ? "sepia hue-rotate-[320deg]" : "brightness-110"}`}
                alt="Fuel"
              />
            </div>
          )}
          <div className="flex-1 flex flex-col gap-1">
            <div className={`${compact ? "h-2" : "h-3"} bg-black/40 rounded-full overflow-hidden border border-white/10`}>
              <div
                ...
                style={{ width: `${percentage}%`, backgroundColor: isRef ? "#daa520" : fuelColor, boxShadow: `0 0 15px ${isRef ? "#daa520" : fuelColor}44` }}
              />
            </div>
          ...
          <div className={`flex items-baseline gap-1 ${compact ? "min-w-[50px]" : "min-w-[80px]"} justify-end`}>
            <span
              className={`${compact ? "text-lg" : "text-2xl"} font-black text-white font-mono drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] tracking-tighter`}
            >
  ...
  ({
    ...
    return (
      <div className="min-w-max flex flex-col gap-3">
              ...
              {hasRef ? (
                showMiniSectors ? (
                  <div className="flex flex-col gap-1.5">
                    <ComparisonRow
                      label={<img src="/finish-flag.png" width={12} height={12} className="opacity-70" alt="Flag" />}
                      val={currentLap?.duration}
                      refVal={(refLap as any)?.duration}
                      compact={true}
                    />
                    {miniSectors.map((seg: any, idx: number) => {
                      ...
                      return (
                        <ComparisonRow
                          ...
                          val={val}
                          refVal={refVal}
                          valColorClass="text-gray-300"
                          compact={true}
                          highlighted={selectedSegIdx === idx}
                        />
                      );
                    })}
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    {[0, 1, 2].map((idx) => {
                      ...
                      const refSectorTimes =
                        hasRef && refLap
                          ? [refLap.s1, refLap.s2, refLap.s3]
                          : [sessionBests?.bestS1?.val, sessionBests?.bestS2?.val, sessionBests?.bestS3?.val];
                      return (
                        <ComparisonRow
                          key={idx}
                          label={`S${idx + 1}`}
                          val={sectorTimes[idx]}
                          refVal={refSectorTimes[idx]}
                          valColorClass="text-gray-300"
                          compact={true}
                          highlighted={selectedSectorIdx === idx}
                        />
                      );
                    })}
                  </div>
                        ...
                        <div className="flex justify-between items-baseline border-b border-white/5 pb-1.5 px-1">
                          <div className="flex items-center gap-2">
                            <span className={`text-[10px] font-black uppercase tracking-widest ${refLap ? "text-yellow-500" : "text-gray-300"}`}>
                              {refLap
                                ? selectedSegIdx !== null
                                  ? `Reference Lap (SEG ${selectedSegIdx + 1})`
                                  : selectedSectorIdx !== null
                                    ? `Reference Lap (S${selectedSectorIdx + 1})`
                                    : "Reference Lap"
                                : selectedSegIdx !== null
                                  ? `Theoretical Best (SEG ${selectedSegIdx + 1})`
                                  : selectedSectorIdx !== null
                                    ? `Session Best (S${selectedSectorIdx + 1})`
                                    : "Theoretical Best"}
                            </span>
                            {refLap && (
                              <span className="px-1.5 py-0.5 rounded-sm bg-yellow-500/20 text-white text-[7px] font-black border border-yellow-500/20 leading-none">
                                L{refLap.lap}
                              </span>
                            )}
                          </div>
                          <span className={`text-[17px] font-black font-mono tracking-tighter ${refLap ? "text-yellow-500" : "text-purple-400"}`}>
                            {formatTime(
                              refLap
                                ? selectedSegIdx !== null
                                  ? refLapMiniSectorTimes?.[selectedSegIdx]?.duration || 0
                                  : selectedSectorIdx !== null
                                    ? [refLap.s1, refLap.s2, refLap.s3][selectedSectorIdx] || 0
                                    : refLap.duration || 0
                                : selectedSegIdx !== null
                                  ? sessionMiniSectorBests?.bests[selectedSegIdx]?.val || 0
                                  : selectedSectorIdx !== null
                                    ? [sessionBests?.bestS1, sessionBests?.bestS2, sessionBests?.bestS3][selectedSectorIdx]?.val || 0
                                    : showMiniSectors
                                      ? sessionMiniSectorBests?.theoreticalBest || 0
                                      : sessionBests?.theoreticalBest || 0
                            )}
                          </span>
                        </div>
                            ...
                            {refLap ? (
                              showMiniSectors ? (
                                refLapMiniSectorTimes?.map((item, idx) => {
                                  ...
                                  return (
                                    <div key={idx} className="flex flex-col p-1 transition-all rounded-lg duration-300">
                                      <span className="text-[8px] font-black text-gray-500 uppercase tracking-[0.15em] mb-0.5">R{idx + 1}</span>
                                      <div className="flex items-baseline gap-1.5">
                                        <span className="text-[12px] font-black text-white font-mono tracking-tighter">{displayTime}</span>
                                      </div>
                                    </div>
                                  );
                                })
                                  ...
                                  {[
                                    {
                                      label: "RS1",
                                      val: refLap?.s1,
                                      isBest: refLap?.s1 > 0 && refLap?.s1 <= (sessionBests?.bestS1.val || 0),
                                      lap: sessionBests?.bestS1.lap || 0
                                    },
                                    {
                                      label: "RS2",
                                      val: refLap?.s2,
                                      isBest: refLap?.s2 > 0 && refLap?.s2 <= (sessionBests?.bestS2.val || 0),
                                      lap: sessionBests?.bestS2.lap || 0
                                    },
                                    {
                                      label: "RS3",
                                      val: refLap?.s3,
                                      isBest: refLap?.s3 > 0 && refLap?.s3 <= (sessionBests?.bestS3.val || 0),
                                      lap: sessionBests?.bestS3.lap || 0
                                    }
                                  ].map((item, idx) => {
                                    ...
                                    return (
                                      <div
                                        key={idx}
                                        className={`flex flex-col p-1 transition-all rounded-lg duration-300 ${selectedSectorIdx === idx ? "bg-purple-500/10 ring-1 ring-purple-500/30" : ""}`}
                                      >
                              ...
                              sessionMiniSectorBests?.bests.map((item: any, idx: number) => {
                                ...
                                return (
                                  <div
                                    key={idx}
                                    className={`flex flex-col p-1 transition-all rounded-lg duration-300 ${selectedSegIdx === idx ? "bg-purple-500/10 ring-1 ring-purple-500/30" : ""}`}
                                  >
                                    <span className="text-[8px] font-black text-gray-500 uppercase tracking-[0.15em] mb-0.5">{item.label}</span>
                                    <div className="flex items-baseline gap-1.5">
                                      <span className="text-[12px] font-black text-white font-mono tracking-tighter">{displayTime}</span>
                              ...
                              [
                                ...
                                return (
                                  <div
                                    key={i}
                                    className={`flex flex-col p-1 transition-all rounded-lg duration-300 ${selectedSectorIdx === i ? "bg-purple-500/10 ring-1 ring-purple-500/30" : ""}`}
                                  >
                                    <span className="text-[8px] font-black text-gray-500 uppercase tracking-[0.15em] mb-0.5">{item.label}</span>
                                    <div className="flex items-baseline gap-1.5">
                                      <span className="text-[12px] font-black text-white font-mono tracking-tighter">{displayTime}</span>
                    ...
                    <div className="flex justify-between items-baseline border-b border-white/5 pb-1.5 px-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">
                          {selectedSegIdx !== null
                            ? `Current Lap (SEG ${selectedSegIdx + 1})`
                            : selectedSectorIdx !== null
                              ? `Current Lap (S${selectedSectorIdx + 1})`
                              : "Current Lap"}
                        </span>
                        {currentLap && (
                      ...
                      <span className="text-[20px] font-black text-white font-mono tracking-tighter drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]">
                        {formatTime(
                          selectedSegIdx !== null
                            ...
                            : selectedSectorIdx !== null
                              ? [currentLap?.s1, currentLap?.s2, currentLap?.s3][selectedSectorIdx] || 0
                              : currentLap?.duration
                        )}
                      </span>
                    </div>
                          ...
                          {miniSectors.map((seg: any, idx: number) => {
                            ...
                            return (
                              <div
                                ...
                                className={`flex flex-col p-1 transition-all rounded-lg duration-300 ${selectedSegIdx === idx ? "bg-purple-500/10 ring-1 ring-purple-500/30" : ""}`}
                              >
                                <span className="text-[8px] font-black text-gray-400 uppercase tracking-[0.15em] mb-0.5">{seg.name}</span>
                                <span className={`text-[13px] font-black font-mono tracking-tighter ${isBest ? "text-purple-400" : "text-yellow-500"}`}>
                                  {displayTime}
                                </span>
                              </div>
                            );
              ...
              <div className="glass-content">
                <div className="flex flex-col gap-1.5">
                  ...
                  {/* <LiveTelemetryRow
                    ...
                    label="Throttle"
                    color={chartConfigs.find((c: any) => c.id === "Throttle Pos" || c.id === "Throttle")?.color || "#00ff00"}
                    unit="%"
        ...
        {hasRef && (
          <div
            className="relative flex items-center p-1 bg-black/30 rounded-xl border border-white/10 self-center w-48 h-8 overflow-hidden group/sync-toggle"
            onMouseMove={handleGlassMouseMove}
          >
            <div
...
{/* 
        {radarInRightColumn && (
          <div
            className={`bg-white/10 glass-container-flat ${hasRef ? "p-2 pt-6" : "p-4 pt-10"} rounded-lg border border-white/25 shadow-xl hover:bg-white/15 transition-all relative group/gforce`}
            onMouseMove={handleGlassMouseMove}
          >
            <div className={`absolute top-0 left-0 w-full flex items-center justify-between ${hasRef ? "p-2" : "p-4"} mix-blend-screen`}>
              <span
                className={`${hasRef ? "text-[8px]" : "text-[10px]"} font-black uppercase tracking-[0.2em] text-gray-500 group-hover/gforce:text-white transition-colors`}
              >
                G-Force Radar
              </span>
            </div>
        ...
        {!radarInRightColumn && (
          <div className="flex justify-center mt-1">
            <div
              className="bg-white/10 glass-container-flat p-2 pt-6 rounded-lg border border-white/25 shadow-xl hover:bg-white/15 transition-all relative group/gforce w-[280px]"
              onMouseMove={handleGlassMouseMove}
            >
              <div className="absolute top-0 left-0 w-full flex items-center justify-between p-2 mix-blend-screen">
                <span className="text-[8px] font-black uppercase tracking-[0.2em] text-gray-500 group-hover/gforce:text-white transition-colors">
                  G-Force Radar
                </span>
              </div>
              <div className="glass-content">
